...
class BackgroundMeshPainter extends CustomPainter {
  ...
  // Pre-allocated static structures
  static const List<Map<String, dynamic>> _driftLeaves = [
    {'startX': 0.05, 'startY': -0.1, 'scale': 0.85, 'baseAngle': pi / 6, 'speedX': 0.08, 'speedY': 0.05, 'swaySpeed': 1.6, 'swayAmp': 20.0, 'bodyColor': Color(0xFF5C6F56), 'outlineColor': Color(0xFF86997F)},
    {'startX': 0.45, 'startY': -0.15, 'scale': 0.90, 'baseAngle': -pi / 6, 'speedX': 0.09, 'speedY': 0.06, 'swaySpeed': 1.8, 'swayAmp': 24.0, 'bodyColor': Color(0xFF4F634A), 'outlineColor': Color(0xFF7D9276)},
  ];

  static const List<Map<String, dynamic>> _lilyPads = [
    {'startX': 0.1, 'startY': 0.3, 'scale': 1.2, 'speed': 0.06, 'color': Color(0xFF43A047), 'rot': 0.2},
    {'startX': 0.7, 'startY': 0.6, 'scale': 0.9, 'speed': 0.08, 'color': Color(0xFF388E3C), 'rot': 1.5},
    {'startX': 0.4, 'startY': 0.8, 'scale': 1.5, 'speed': 0.05, 'color': Color(0xFF2E7D32), 'rot': -0.5},
    {'startX': 0.8, 'startY': 0.1, 'scale': 0.7, 'speed': 0.09, 'color': Color(0xFF66BB6A), 'rot': 0.8},
  ];

  static final List<double> _randomPositions = List.generate(20, (i) => (i * 37 + 13) % 100 / 100.0);
  ...
  final Paint _godRayPaint = Paint()..style = PaintingStyle.fill;
  final Paint _waterfallBgPaint = Paint()..color = const Color(0xFF0B140A).withValues(alpha: 0.85);
  final Paint _waterfallStreamPaint = Paint()
    ..color = const Color(0xFFB3E5FC).withValues(alpha: 0.15)
    ..strokeWidth = 3.0
    ..style = PaintingStyle.stroke
    ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 1.8);
  final Paint _fireflyPaint = Paint();
  final Paint _padPaint = Paint()..style = PaintingStyle.fill;

  final Path _wavePath = Path();
  final Path _leafPath = Path();
  final Path _birdPath = Path();
  final Path _godRayPath = Path();
  final Path _waterfallPath = Path();
  final Path _lilyPadPath = Path();
  ...
  void _drawPacificSunset(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      final double bx = size.width * (0.2 + _randomPositions[i] * 0.6);
      final double by = size.height * (0.1 + _randomPositions[i + 3] * 0.15);
      final double bSize = 6.0 + _randomPositions[i + 6] * 4.0;
  ...
  void _drawSloshingWaves(Canvas canvas, Size size) {
    ...
    for (int layer = 0; layer < 3; layer++) {
      ...
      if (layer == 0) {
        ...
        for (int j = 0; j < 10; j++) {
          final double gx = _randomPositions[j] * size.width;
          final double gy = verticalOffset + (_randomPositions[j + 5] - 0.5) * amplitude;
  ...
  void _drawGodRays(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      _godRayPath.reset();
      _godRayPath.moveTo(p1.dx, p1.dy);
      _godRayPath.lineTo(p2.dx, p2.dy);
      _godRayPath.lineTo(p3.dx, p3.dy);
      _godRayPath.lineTo(p4.dx, p4.dy);
      _godRayPath.close();

      _godRayPaint.shader = LinearGradient(
        begin: Alignment.topCenter, 
        end: Alignment.bottomCenter, 
        colors: [const Color(0xFFFFFDE7).withValues(alpha: 0.10), const Color(0xFFFFF59D).withValues(alpha: 0.0)]
      ).createShader(Rect.fromLTWH(0, 0, size.width, size.height));
      
      canvas.drawPath(_godRayPath, _godRayPaint);
    }
  }
  ...
  void _drawWaterfall(Canvas canvas, Size size) {
    ...
    canvas.drawRect(rect, _waterfallBgPaint);

    for (int i = 0; i < 3; i++) {
      final double streamX = (i + 0.5) * (waterfallWidth / 3.0);
      _waterfallPath.reset();
      _waterfallPath.moveTo(streamX, 0);
      for (double y = 0; y <= size.height; y += 30.0) {
        _waterfallPath.lineTo(streamX + sin(y * 0.06 - time * 32.0 + i * 2.5) * 2.8, y);
      }
      canvas.drawPath(_waterfallPath, _waterfallStreamPaint);
    }
  }

  void _drawFireflies(Canvas canvas, Size size) {
    for (int i = 0; i < 6; i++) {
      final double px = (_randomPositions[i] * 1.2 - 0.1) * size.width + sin(time * 2 * pi + i) * 10.0;
      final double py = (_randomPositions[i + 6] * 1.2 - 0.1) * size.height;
      final double pulse = 0.25 + (sin(time * 3.5 * pi + i * 1.8) + 1.0) / 2.0 * 0.75;
      _fireflyPaint.color = const Color(0xFFFFFF8D).withValues(alpha: 0.82 * pulse);
      canvas.drawCircle(Offset(px, py), 2.2, _fireflyPaint);
    }
  }
  ...
  void _drawRiverSurface(Canvas canvas, Size size) {
    ...
    for (int layer = 0; layer < 5; layer++) {
      final double waveSpeed = 0.5 + (layer * 0.2);
    ...
    for (var pad in _lilyPads) {
      ...
      _padPaint.color = pad['color']!;
      
      canvas.save();
      canvas.translate(px, py);
      canvas.rotate(pad['rot']! + time * 0.2);
      
      _lilyPadPath.reset();
      _lilyPadPath.addArc(Rect.fromCircle(center: Offset.zero, radius: 15.0 * pad['scale']!), 0.4, 2 * pi - 0.8);
      _lilyPadPath.lineTo(0, 0);
      _lilyPadPath.close();
      
      canvas.drawPath(_lilyPadPath, _padPaint);
      canvas.restore();
    }
  }
